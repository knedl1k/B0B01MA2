...
$\vec{F}(x, y, z) =
\begin{pmatrix}
    z^2 -x, & -2xy, & \frac{3z}{1+x^2}
\end{pmatrix}$
hranicí tělesa \\ omezeného plochami $z = 4 - y^2$, $z=0$, $x=0$ a $x=3$ s vnější orientací.
...
\end{multicols}
\[
\iiint\limits_{(M)} \text{div} \VF{F} \dif \VF{S} = \int_{0}^{3} \int_{-2}^{2} \int_{0}^{4-y^2} -1 -2x + \frac{3}{1+x^2} 
