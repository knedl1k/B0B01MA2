...
Ukažte, že práce síly 
$\vec{F} = 
\begin{pmatrix}
    2xz + \sin y, & x \cos y + z, & x^2+y
\end{pmatrix}$
podél křivky z $[0, 1, 0]$ do $[1, 0, 1]$ nezávisí na křivce a najděte její hodnotu.
...
\begin{multicols}{2}
    ...
    Polární souřadnice:
    \begin{flalign*}
        E\text{: } -\frac{\pi}{2} &\leq \varphi \leq \frac{\pi}{2} & \\
        0 &\leq \varrho \leq 2 \cos \varphi
    \end{flalign*}
    $$\int_{-\frac{\pi}{2}}^{\frac{\pi}{2}} \int_{0}^{2 \cos \varphi} \varrho f(\varrho \cos \varphi, \varrho \sin \varphi) \dif \varrho \dif \varphi$$
...
$\iint\limits_{(M)} \text{rot} \vec{F} \dif \vec{S}$, kde  
$\vec{F} (x, y, z) =
\begin{pmatrix}
    xy^2, & -x^2 y, & xyz
\end{pmatrix}$
a $M$ je část plochy $z(x,y) = 1- x^2 - y^2$ nad rovinou $xy$ orientovaná nahoru.
